...
class _GroupprofileState extends State<Groupprofile> {
  ...
  void getgroupdesc() async {
    final docs = await Firestore.instance.collection('Groups').document(widget.groupid).collection('description').getDocuments();
  ...
  void getgroupimage() async{
    final docs = await Firestore.instance.collection('Groups').document(widget.groupid).collection('about').getDocuments();
...
//  }
  Future uploadpic(BuildContext content)async{
    final StorageReference firebaseref=await FirebaseStorage.instance.ref().child('grouppictures');
    StorageUploadTask task = await firebaseref.child("pic"+DateTime.now().toString()).putFile(picture);
    ...
    return Scaffold(
      body:CustomScrollView(
        ...
        flexibleSpace: FlexibleSpaceBar(
          ...
          background: photo
              ? Image.file(picture): Image.network(grpurl, fit:BoxFit.cover),
        ),),
      SliverFillRemaining(
        child: Column(
          children: <Widget>[
      ...
      child: ListTile(
        ...
        title: GestureDetector(
          onTap: (){ Navigator.pushReplacement( context, MaterialPageRoute(
            builder: (context) => chatscreen(contact: widget.user, ), ),
          ); },
          child: Container(
            child: Text(name,
              style: TextStyle(
