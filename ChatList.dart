...
class _ChatlistState extends State<Chatlist> {
  ...
  groupchat(groupname)async {
   final grpname= await  Firestore.instance.collection('chatlists').document(email).collection('groups').where('Groupname', isEqualTo: groupname).getDocuments();
   var grpid=grpname.documents[0].data['Groupid'];
   final grpprof = await Firestore.instance.collection('Groups').document(grpid).collection('about').getDocuments();
  ...
  personalchat(frndsmail)async{
    final data = await Firestore.instance.collection('users') .where('mail', isEqualTo: frndsmail).getDocuments();
  ...
  void getUser() async {
    await auth.currentUser().then((user) async {
      final url = await list.collection('users').where('mail', isEqualTo: user.email).getDocuments();
      setState(() {
        email = user.email;
                          ...
                          child: Row(mainAxisAlignment:MainAxisAlignment.spaceEvenly,
                            children: <Widget>[
                              GestureDetector(
                                ...
                                child:Container(width:100,child: Icon(Icons.message)),
                              ),
                              GestureDetector(
                                onTap:(){ Navigator.pushReplacement( context, MaterialPageRoute(
                                    builder: (context) => frndprofile(name:user,profileurl:url,mail: mail) ),
                                ); },
                                child: Container(width:100,child: Icon(Icons.info_outline)),
                              )
