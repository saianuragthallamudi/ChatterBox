...
class _profileState extends State<profile> {
  ...
  getdata() async{
    ...
    final userinfo =await Firestore.instance.collection("users").where('uid',isEqualTo: user.uid).getDocuments();
    setState(() {
  ...
  Future uploadpic(BuildContext content)async{
    final StorageReference firebaseref=await FirebaseStorage.instance.ref().child('profilepictures');
    StorageUploadTask task = await firebaseref.child("pic"+DateTime.now().toString()).putFile(profilepic);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text("chatter_box"),),
      body: Container(
          child:Column(
            ...
            children:<Widget>[
              ...
              Row(
                children: <Widget>[
                  Padding(
                    ...
                    child: Container(child: Icon(Icons.person_pin_circle),)),
                  Expanded(
                    child:Column(crossAxisAlignment:CrossAxisAlignment.start,mainAxisAlignment:MainAxisAlignment.start,
                    children: <Widget>[
                     Text('nickname',style: TextStyle(color:Colors.black38,fontSize: 15,fontWeight:FontWeight.w300),),
                      
                     Text(name,style: TextStyle(color:Colors.black,fontSize: 20),)
                    ],)
                  ),
                  Padding( padding: EdgeInsets.symmetric(horizontal:30),child: GestureDetector(onTap:(){
                    Alert(
                     context: context,
                      title: "NickName",
                      content: TextField(
                     ...
                     onPressed: () async { 
                        await FirebaseAuth.instance.currentUser().then((user){
                           ...
                           Firestore.instance.document('/users/${docs.documents[0].documentID}').updateData({'nickname':editname.text});
                          });
                        });
              ...
              Row(
                children: <Widget>[
                  Padding(
                    padding: EdgeInsets.symmetric(horizontal:30),
                    child: Container(child: Icon(Icons.perm_identity),)),
                  Expanded(
                    child:Column(crossAxisAlignment:CrossAxisAlignment.start,mainAxisAlignment:MainAxisAlignment.start,
                    children: <Widget>[
                     Text('bio',style: TextStyle(color:Colors.black38,fontSize: 15,fontWeight:FontWeight.w300),),
                  ...
                  child: GestureDetector(onTap:(){
                    Alert(
                     ...
                     onPressed: () async { 
                        await FirebaseAuth.instance.currentUser().then((user){
                           Firestore.instance.collection('/users').where('uid',isEqualTo: user.uid).getDocuments().then((docs) {
                           Firestore.instance.document('/users/${docs.documents[0].documentID}').updateData({'Bio':editbio.text});
                          });
                        });
                      Navigator.pop(context);
                      },
               ...
               Row(
                children: <Widget>[
                  ...
                  Expanded(
                    child:Column(crossAxisAlignment:CrossAxisAlignment.start,mainAxisAlignment:MainAxisAlignment.start,
                    children: <Widget>[
                     Text('mail',style: TextStyle(color:Colors.black38,fontSize: 15,fontWeight:FontWeight.w300),),
                      
                     Text(mail,style: TextStyle(color:Colors.black,fontSize: 20),)
                    ],)
                  ),
             ...
             SizedBox(height: 50,),
              Row(
                ...
                children: <Widget>[
                  ...
                  elevation:5.0,
                  child:Text('Update',style: TextStyle(color:Colors.black,fontSize: 15),),
                  )
